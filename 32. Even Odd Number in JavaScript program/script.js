...
জাভাস্ক্রিপ্টে একটি অদ্ভুত বিষয় হলো, যদি সরাসরি চেক করা হয় NaN == NaN, তবে উত্তর আসবে false। 
...
isNaN() এর কাজ হলো  কোডকে ভুল ডাটা বা টেক্সট ইনপুট থেকে বাঁচানো এবং নিশ্চিত করা যে যা নিয়ে কাজ  করা হসছে সেটি আসলেই একটি সংখ্যা।

console.log(isNaN(Number(null))); // Output: false
কারণ Number(null) এর মান হলো 0, আর 0 তো কোনো NaN নয়, এটি একটি বৈধ সংখ্যা। তাই ফলাফল আসে false।

isNaN(Number(undefined)) 
// ধাপ ১: Number(undefined) পরিবর্তিত হয়ে হলো NaN
// ধাপ ২: isNaN(NaN) চেক করলো এটি কি NaN?
// ফলাফল: true

এটি true আসার কারণ হলো জাভাস্ক্রিপ্টে undefined-কে যখন সংখ্যায় রূপান্তর (Type Conversion) করা হয়, তখন তার ফলাফল হয় NaN।
জাভাস্ক্রিপ্টের নিয়ম অনুযায়ী, undefined-কে সংখ্যায় রূপান্তর করলে সেটি কোনো গাণিতিক মান দিতে পারে না। তাই এর ফলাফল সরাসরি NaN হয়।

Number(undefined) -- NaN
Number(null) --- 0
isNaN ফাংশনটির কাজ হলো চেক করা যে, ব্র্যাকেটের ভেতরের মানটি NaN কি না। যেহেতু ইনপুট হিসেবে NaN, তাই সে উত্তর দিচ্ছে true।

জাভাস্ক্রিপ্টে null মানে হলো "খালি বা শূন্য মান" (তাই একে ০ ধরা হয়), কিন্তু undefined মানে হলো "মানটি এখনো নির্ধারিতই হয়নি" (Missing Value)। যেহেতু মানটি নেই,
তাই এটি দিয়ে কোনো গাণিতিক হিসাব সম্ভব নয়। আর গাণিতিকভাবে যা অসম্ভব, সেটিই হলো NaN।

----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
কেন NaN === NaN সবসময় False হয়?
    
জাভাস্ক্রিপ্টে NaN (Not-a-Number) কোনো নির্দিষ্ট মান (Value) নয়, বরং এটি একটি অবস্থা (State)। এটি নির্দেশ করে যে কোনো একটি গাণিতিক হিসাব ভুল হয়েছে। 
যেহেতু প্রতিটি গাণিতিক ভুল আলাদা হতে পারে, তাই একটি NaN কখনো অন্যটির সমান হয় না।

হিসাব 'ক': "Hello" / 5  ফলাফল: NaN
হিসাব 'খ': undefined + 10 ফলাফল: NaN

এখানে 'ক' এবং 'খ' উভয়ই NaN দিচ্ছে। কিন্তু  "Hello" / 5 আর undefined + 10 আসলে একই জিনিস? অবশ্যই না।
সিদ্ধান্ত: যেহেতু এদের উৎস আলাদা, তাই জাভাস্ক্রিপ্ট এদেরকে সমান (===) বলে না।
টেকনিক্যাল কারণ (IEEE 754 Standard)
জাভাস্ক্রিপ্ট সংখ্যা গণনার জন্য আন্তর্জাতিক IEEE 754 নিয়ম মেনে চলে। এই নিয়ম অনুযায়ী:

"NaN এর সাথে যেকোনো তুলনা (Comparison) সবসময় false হবে, এমনকি সে যদি নিজের সাথেও তুলনা করা হয়।"

এটি একটি গ্লোবাল স্ট্যান্ডার্ড যা কম্পিউটার বিজ্ঞানীরা ঠিক করে দিয়েছেন যাতে হিসেবে বড় কোনো ভুল না হয়।


চেক করার উপায়: NaN চেক করার জন্য কখনো == বা === ব্যবহার করা যাবে না। সবসময় isNaN() ফাংশন ব্যবহার করতে হবে।
let value = NaN;
console.log(value === NaN); // Output: false (ভুল পদ্ধতি)
console.log(isNaN(value));  // Output: true (সঠিক পদ্ধতি)

let myValue = NaN;
console.log(myValue === NaN); // ভুল পদ্ধতি (Output: false)
console.log(isNaN(myValue));  // সঠিক পদ্ধতি (Output: true)

NaN === NaN	------ false  কারণ NaN মানেই হলো "অজানা ত্রুটি", আর দুটি অজানা জিনিস সমান হতে পারে না।

-----------------------------------
দুইজন বন্ধুকে আলাদা আলাদা রুমে ডেকে নিয়ে বলা হল— "মনে মনে একটি সংখ্যা ধরো যা 'সংখ্যা নয়' (Not-a-Number)"।

বন্ধু ১ মনে মনে ভাবলো: একটি "আম" (এটি তো আর সংখ্যা না)।

বন্ধু ২ মনে মনে ভাবলো: একটি "পাখি" (এটিও সংখ্যা না)।
দুই বন্ধুর ভাবা জিনিস দুটি কি সমান?" "না"। কারণ তারা দুজনেই যদিও "সংখ্যা নয়" এমন কিছু ভেবেছে, কিন্তু তাদের সেই "জিনিস" দুটি এক নয়।
জাভাস্ক্রিপ্টেও ঠিক এই ঘটনা ঘটে। একেকটি NaN একেকটি গাণিতিক ভুলের কারণে তৈরি হতে পারে, তাই কম্পিউটার নিশ্চিত হতে পারে না যে তারা সমান কি না।

"Hello" / 5 এর ফলাফল হলো NaN

Math.sqrt(-1) এর ফলাফল হলো NaN

Hello" / 5 আর Math.sqrt(-1) কি এক জিনিস? অবশ্যই না।

যেহেতু দুটি আলাদা কারণে NaN তৈরি হতে পারে এবং তাদের আসল মান কী তা  জানি না, তাই জাভাস্ক্রিপ্ট ধরে নেয় যে একটি NaN কখনোই অন্য একটি NaN-এর সমান হতে পারে না।



let result = "abc" / 2;
if (result === NaN) { // এটি কখনোই কাজ করবে না, কারণ false আসবে
    console.log("এটি NaN");
}

let result = "abc" / 2;
if (isNaN(result)) { // এটি সঠিকভাবে true দিবে
    console.log("এটি একটি অবৈধ সংখ্যা বা NaN");
}
